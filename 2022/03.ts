...
const packOfGifts2 = ["book", "doll", "ball"]
const reindeers2 = ["dasher", "dancer"]

// el pack de regalos pesa 4 + 4 + 4 = 12
// los renos pueden llevar (2 * 6) + (2 * 6) = 24
// por lo tanto, Santa Claus puede entregar 2 cajas de regalos

const distributeGiftsResult2 = distributeGifts2(packOfGifts2, reindeers2) // 2
...
function distributeGifts2(packOfGifts: string[], reindeers: string[]) {
  const packWeight = packOfGifts.reduce((acc, gift) => {
    return acc + gift.length;
  }, 0);

  const reindeersWeight = reindeers.reduce((acc, reindeer) => {
